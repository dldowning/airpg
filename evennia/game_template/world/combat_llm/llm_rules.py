...
OLLAMA_MODEL = "llama3"  # Or whatever model is being used
...
class LLMCombatRules:
    ...
    def _call_ollama(self, prompt_text):
        ...
        Returns:
            dict or None: The parsed JSON response from the LLM, or None if an
                          error occurred.
        """
        try:
            ...
            # We are interested in the final one which has the full response object.
            lines = response.text.strip().split('\n')
            final_line = lines[-1]
            response_data = json.loads(final_line)

            # The actual JSON content we want is inside the 'response' key,
            # which is a string that needs to be parsed again.
            return json.loads(response_data.get("response", "{}"))

        except requests.exceptions.RequestException as e:
            print(f"|r[LLMCombatRules] Error connecting to Ollama: {e}|n")
            return None
        except json.JSONDecodeError as e:
            print(f"|r[LLMCombatRules] Error decoding JSON from Ollama: {e}|n")
            print(f"|rRaw response was: {response.text}|n")
            return None
