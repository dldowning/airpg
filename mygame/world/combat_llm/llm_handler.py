...
# Manual, low-level logging for debugging startup crashes
def log_error(message):
    with open("/app/debug.log", "a") as f:
        f.write(f"{message}\n")

log_error("llm_handler.py: Top of file")

try:
    from evennia import DefaultScript
    log_error("llm_handler.py: Imported DefaultScript successfully.")
    if DefaultScript is None:
        log_error("llm_handler.py: DefaultScript is None!")
except Exception as e:
    log_error(f"llm_handler.py: FAILED to import DefaultScript: {e}")
    sys.exit(1) # Exit immediately to be sure

try:
    from .llm_rules import roll_d20, roll_dice
    log_error("llm_handler.py: Imported llm_rules successfully.")
except Exception as e:
    log_error(f"llm_handler.py: FAILED to import llm_rules: {e}")
    sys.exit(1)
